...
def _draw_stats_card(ax, result: AnalysisResult) -> None:
    """Draw a text-based stats card with scores and augmentation list."""
    q = result.quality
    c = result.content
...
def generate_image_report(result: AnalysisResult, output_dir: str) -> str:
    """Generate a 3-panel visual report PNG for a single image.

    Parameters
    ----------
    result : AnalysisResult
        Full analysis result for the image.
    output_dir : str
        Directory where the report PNG will be saved.
