# modules/reporting/dataset_analytics.py
...
Produces:
  ...
  - Duplicate detection via perceptual hashing + SSIM (if scikit-image is installed).
...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend — required for Flask worker threads
import matplotlib.pyplot as plt

from modules.utils.models import AnalysisResult
...
def _plot_distributions(results: list[AnalysisResult], output_dir: str) -> str:
    """Generate a histogram grid and save to PNG."""
    ...
    for idx, (title, values) in enumerate(metrics.items()):
        ...
        ax.hist(values, bins=max(5, len(values) // 2), color="#7c4dff",
                edgecolor="#b388ff", alpha=0.85)
...
# ---------------------------------------------------------------------------
# Embedding preview (PCA)
# ---------------------------------------------------------------------------

def _embedding_preview(results: list[AnalysisResult], output_dir: str) -> str:
    ...
    fig.patch.set_facecolor("#121212")
    ax.set_facecolor("#1e1e2f")
    ax.scatter(coords[:, 0], coords[:, 1], c=colours, s=120,
               edgecolors="white", linewidths=0.5, zorder=5)

    for i, name in enumerate(names):
        ax.annotate(name[:20], (coords[i, 0], coords[i, 1]),
                    fontsize=7, color="#ccc", xytext=(5, 5),
    ...
    ax.legend(handles=legend_elements, loc="upper right",
              facecolor="#2a2a40", edgecolor="#444", labelcolor="#e0e0e0")
...
def analyze_dataset(results: list[AnalysisResult], output_dir: str) -> dict:
    """Run all dataset-level analytics and return a summary dictionary.

    Parameters
    ----------
    results : list[AnalysisResult]
        List of per-image analysis results.
    output_dir : str
    ...
    -------
    dict
        Keys: ``summary``, ``distribution_plot``, ``duplicates_csv``,
        ``embedding_plot``.
    """
